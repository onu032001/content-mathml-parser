class ContentMathMLParser {
  ...
  extractArgs(args) {
    const arrayArgs = args.filter(item => item instanceof Object);
    const objectArgs = Object.assign({}, ...args.filter(item => item instanceof Object));
  ...
  parseFromString(mathml) {
    try {
      return this.domParser.parseFromString(mathml, "application/xml");
    } catch (error) {
